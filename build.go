...
func main() {
	if _, err := os.Stat("assets"); err != nil {
		log.Fatalf("Error: cannot find chesseract assets directory. (error: %s)\nAre you running this from the repository root?", err)
	}
	...
	flag.BoolVar(&run, "run", false, "Run chesseract upon successful compilation")
	flag.Parse()
...
func compile(ctx context.Context, conf compileConfig) error {
	...
	if err := os.Chdir("assets"); err != nil {
		return errors.Errorf("Error: cannot find chesseract assets directory. (error: %s)\nAre you *sure* you're running this from the repository root?", err)
	}
	var emb resemble.Resemble
	emb.OutputFile = "../internal/assets/assets.go"
	emb.PackageName = "assets"
	...
	if conf.PackageVersion == "" {
		...
		if err != nil || len(gitDescribe) == 0 {
			conf.PackageVersion = "default_version"
		} else {
